...
function applyCalendarDatePick(dateValue) {
    if (!dateValue) return;
    if (calendarPickMode === 'date1' && byId('date-use-now')?.checked) return;
...
export function renderCalendar() {
    ...
    let html = '';
    const wd = getLocale().startsWith('ru') ? ['Вс', 'Пн', 'Вт', 'Ср', 'Чт', 'Пт', 'Сб'] : ['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'];
    ...
    for (let day = 1; day <= totalDays; day++) {
        ...
        const isDate1 = d1Selected && dayDate.toDateString() === d1Selected.toDateString();
        const isDate2 = d2Selected && dayDate.toDateString() === d2Selected.toDateString();
        const inRange = rangeStart && rangeEnd && dayDate > rangeStart && dayDate < rangeEnd;
    ...
    byId('calendar-days').querySelectorAll('.calendar-day-clickable').forEach(cell => {
