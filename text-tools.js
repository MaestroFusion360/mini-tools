...
function formatReadTime(readMins) {
    ...
    if (totalMins < 60) return getLanguage() === 'ru' ? `${totalMins} мин` : `${totalMins} min`;
    const h = Math.floor(totalMins / 60);
    const m = totalMins % 60;
    return m ? (getLanguage() === 'ru' ? `${h} ч ${m} мин` : `${h} h ${m} min`) : (getLanguage() === 'ru' ? `${h} ч` : `${h} h`);
}
...
function renderTextMetrics(metrics) {
    ...
    if (el) el.innerHTML = (getLanguage() === 'ru' ? ruLines : enLines).join('<br>');
}
...
export function toTitleCaseText() {
    applyTextTransform(text =>
        text.toLowerCase().replace(/(^|[\s([{"'\-])([A-Za-z\u0400-\u04FF])/g, (_, p1, p2) => `${p1}${p2.toUpperCase()}`)
    );
}

export function toSentenceCaseText() {
    applyTextTransform(text =>
        text.toLowerCase().replace(/(^\s*[A-Za-z\u0400-\u04FF])|([.!?]\s+[A-Za-z\u0400-\u04FF])/g, match => match.toUpperCase())
    );
}
