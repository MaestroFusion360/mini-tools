...
function renderCalcDisplay() {
    ...
    byId('calc-expression-preview').textContent = `${t('calcExpression')}: ${calcVal}`;
}
...
export function toggleCalcMode(toggle = true) {
    ...
    byId('calc-mode-btn').textContent = calcScientificMode ? t('calcModeScientific') : t('calcModeBasic');
...
function applyCalculatorTranslations() {
    ...
    setText('calc-mode-btn', calcScientificMode ? t('calcModeScientific') : t('calcModeBasic'));
